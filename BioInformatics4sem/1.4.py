from enum import Enum #inclorrect((
...
for i in range(1, t_len):
    for j in range(1, s_len):
        if (upper[i - 1][j][0] + gap_continue >= main[i - 1][j][0] + gap_start + gap_continue):
            upper[i][j] = [upper[i - 1][j][0] + gap_continue, Transition.DOWN]
        else:
            upper[i][j] = [main[i - 1][j][0] + gap_start + gap_continue, Transition.FROM_MAIN]
        if (lower[i][j - 1][0] + gap_continue >= main[i][j - 1][0] + gap_start + gap_continue):
            lower[i][j] = [lower[i][j - 1][0] + gap_continue, Transition.RIGHT]
        else:
            lower[i][j] =  [main[i][j - 1][0] + gap_start + gap_continue, Transition.FROM_MAIN]
        main_cell_value = main[i - 1][j - 1][0] + (match if s[j - 1] == t[i - 1] else mismatch)
        main_cell = max(main_cell_value, upper[i][j][0], lower[i][j][0])
        if (main_cell == main_cell_value):
            main[i][j] = [main_cell, Transition.MAIN]
        elif (main_cell == upper[i][j][0]):
            main[i][j] = [main_cell, Transition.FROM_UPPER]
            upper[i][j][1] = Transition.TO_MAIN
        else:
            main[i][j] =[main_cell, Transition.FROM_LOWER]
            lower[i][j][1] = Transition.TO_MAIN
...
while i > 0 or j > 0:
    ...
    elif transition == Transition.FROM_UPPER:
        flag = Transition.DOWN
    elif transition == Transition.DOWN:
        i -= 1
        s_aligned.append('-')
        t_aligned.append(t[i])
    elif transition == Transition.FROM_LOWER:
        flag = Transition.RIGHT
    elif transition == Transition.RIGHT:
        j -= 1
        s_aligned.append(s[j])
        t_aligned.append('-')
    elif transition == Transition.FROM_MAIN and flag == Transition.DOWN:
        i -= 1
        s_aligned.append('-')
        t_aligned.append(t[i])
        flag = Transition.MAIN
    elif transition == Transition.FROM_MAIN and flag == Transition.RIGHT:
        j -= 1
        s_aligned.append(s[j])
        t_aligned.append('-')
        flag = Transition.MAIN
...
# for i in range(1, t_len):
#     for j in range(1, s_len):
#         upper[i][j] = [upper[i - 1][j][0] + gap_continue, Transition.DOWN] if upper[i - 1][j][0] + gap_continue >= main[i - 1][j][0] + gap_start + gap_continue else [main[i - 1][j][0] + gap_start + gap_continue, Transition.TO_MAIN]
#         lower[i][j] = [lower[i][j - 1][0] + gap_continue, Transition.RIGHT] if lower[i][j - 1][0] + gap_continue >= main[i][j - 1][0] + gap_start + gap_continue else [main[i][j - 1][0] + gap_start + gap_continue, Transition.TO_MAIN]
#         main_cell_value = main[i - 1][j - 1][0] + (match if s[j - 1] == t[i - 1] else mismatch)
#         main_cell = max(main_cell_value, upper[i][j][0], lower[i][j][0])
#         main[i][j] = [main_cell, Transition.MAIN if main_cell == main_cell_value else (Transition.TO_UPPER if main_cell == upper[i][j][0] else Transition.TO_LOWER)]
