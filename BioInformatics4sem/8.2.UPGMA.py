    ...
    def merge_clusters(self, indices):
        ...
        for i in range(len(self.distances)):
            if i != first and i != second:
                first_weight, second_weight = self.clusters[first].size, self.clusters[second].size
                self.distances[i][first] \
                    = self.distances[first][i] \
                    = ((self.distances[i][first] * first_weight + self.distances[i][second] * second_weight) /
